...
def date_conversion(df, varDate):
    ...
    """
    # Transforming the variable into a datetime
    df[varDate] = pd.to_datetime(df[varDate])

    # strftime() method can be used to create formatted string
    df['week-year'] = df[varDate].dt.strftime('%V-%Y')
    
    # Extracting the week of the year from the ORDERDATE
    df['week'] = df[varDate].dt.isocalendar().week
